...
namespace Client
{
    ...
    public partial class MainWindows : Form
    {
        ...
        public double maxDelay = 10;
        public double minDelay = 2;
        public double maxUpDelay = 20;
        public double minUpDelay = 2;


        //用于上下行、是否TSN时延抖动(根据上下行、优先级组合求差值)
        public double upLightGap;
        public double upMotorGap;
        public double downLightGap;
        public double downMotorGap;
        public double downLightAvg;
        public double downMotorAvg;

        public double upLightAvg;
        public double upMotorAvg;
        ...
        */
                ...
                while (reader.Read())
                {
                    ...
                    if (reader[0].ToString() != "" && reader[1].ToString() != "")
                    {
                        ...
                        */
                        ...
                        max = Convert.ToDouble(reader["maxDelay"].ToString());
                        min = Convert.ToDouble(reader["minDelay"].ToString());
        ...
        private void getGapData()
        {
            ...
            myConnect.Open();
            //下行
            MySqlCommand downLightCmd = new MySqlCommand("SELECT STDDEV_SAMP(delay) as gap from time where upFlag=0 AND date_sub(now(), interval 1 minute) < time AND priority =?lightP;", myConnect);
            MySqlCommand downMotorCmd = new MySqlCommand("SELECT STDDEV_SAMP(delay) as gap from time where upFlag=0 AND date_sub(now(), interval 1 minute) < time AND priority =?motorP;", myConnect);
            downLightCmd.Parameters.AddWithValue("@lightP", lightP);
            downMotorCmd.Parameters.AddWithValue("@motorP", motorP);

            //下行平均
            MySqlCommand downAvgCmd = new MySqlCommand("SELECT AVG(delay) as avg from time where upFlag=0 AND date_sub(now(), interval 1 minute) < time AND priority =?lightP;SELECT AVG(delay) as avg from time where upFlag=0 AND date_sub(now(), interval 1 minute) < time AND priority =?motorP;", myConnect);
            downAvgCmd.Parameters.AddWithValue("@lightP", lightP);
            downAvgCmd.Parameters.AddWithValue("@motorP", motorP);

            //上行
            MySqlCommand upLightCmd = new MySqlCommand("SELECT STDDEV_SAMP(delay) as gap from time where upFlag=1 AND date_sub(now(), interval 1 minute) < time AND priority =?lightP;", myConnect);
            MySqlCommand upMotorCmd = new MySqlCommand("SELECT STDDEV_SAMP(delay) as gap from time where upFlag=1 AND date_sub(now(), interval 1 minute) < time AND priority =?motorP;", myConnect);
            upLightCmd.Parameters.AddWithValue("@lightP", 0);
            upMotorCmd.Parameters.AddWithValue("@motorP", 7);
            ...
            {
                //1、以ExecuteReader来执行sql--解决了判断是否为空的问题，但是不方便数据类型转换
                MySqlDataReader reader = downLightCmd.ExecuteReader();
                //Console.WriteLine("\n\n--------------------------");
                //Console.WriteLine("reader.HasRows:" + reader.HasRows);
                while (reader.Read())
                {
                    ...
                    if (reader[0].ToString() != "")
                    {
                        ...
                        */
                        ...
                        downLightGap = Convert.ToDouble(reader["gap"].ToString());
                        //Console.WriteLine("min:" + min);
                        //Console.WriteLine("max:" + max);
                        //max = ((max / 10) + 1) * 10;
                        //min = (min / 10) * 10;
                ...
                MySqlDataReader reader2 = downMotorCmd.ExecuteReader();
                while (reader2.Read())
                {
                    //当有最值数据时才替换设置范围
                    if (reader2[0].ToString() != "")
                    {
                        downMotorGap = Convert.ToDouble(reader2["gap"].ToString());
                    }
                    reader2.NextResult();
                }
                reader2.Close();
                ...
                MySqlDataReader readerAvg = downAvgCmd.ExecuteReader();
                int i = 1;
                while (readerAvg.Read())
                {
                    i = i % 2;
                    //当有最值数据时才替换设置范围
                    if (readerAvg[0].ToString() != "" && i == 1)
                    {
                        downLightAvg = Convert.ToDouble(readerAvg["avg"].ToString());
                    }
            ...
            {
                //1、以ExecuteReader来执行sql--解决了判断是否为空的问题，但是不方便数据类型转换
                MySqlDataReader reader = upLightCmd.ExecuteReader();
                //Console.WriteLine("\n\n--------------------------");
                //Console.WriteLine("reader.HasRows:" + reader.HasRows);
                while (reader.Read())
                {
                    ...
                    if (reader[0].ToString() != "")
                    {
                        ...
                        */
                        ...
                        upLightGap = Convert.ToDouble(reader["gap"].ToString());
                        //Console.WriteLine("min:" + min);
                        //Console.WriteLine("max:" + max);
                        //max = ((max / 10) + 1) * 10;
                        //min = (min / 10) * 10;
                ...
                MySqlDataReader reader2 = upMotorCmd.ExecuteReader();
                while (reader2.Read())
                {
                    //当有最值数据时才替换设置范围
                    if (reader2[0].ToString() != "")
                    {
                        upMotorGap = Convert.ToDouble(reader2["gap"].ToString());
                    }
                    reader2.NextResult();
                }
                reader2.Close();
                ...
                MySqlDataReader readerAvgUp = upAvgCmd.ExecuteReader();
                int i = 1;
                while (readerAvgUp.Read())
                {
                    i = i % 2;
                    //当有最值数据时才替换设置范围
                    if (readerAvgUp[0].ToString() != "" && i == 1)
                    {
                        upLightAvg = Convert.ToDouble(readerAvgUp["avg"].ToString());
                    }
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            if (isStart == true) //想要开始接收时
            {
                try
                {
                    localIpep = new IPEndPoint(IPAddress.Parse(srcIp), 12345); //本机IP和监听端口号
