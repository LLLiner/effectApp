...
namespace Client
{
    public partial class wireless : Form
    {
        ...
        //根据显示时延数据
        private DataTable getDBDataByFlag(int interp)
        {
            int interP1 = interp;
            int interP2 = interp + 20;

            MySqlConnection myConnect = new MySqlConnection(constructorString);
            myConnect.Open();
            MySqlCommand myCmd = new MySqlCommand("SELECT * from wireless WHERE id > ?interP1 AND id < ?interP2 and  upFlag=2;", myConnect);
        ...
        {
            //重新获取下行时延数据，绑定数据
            DataTable tsnDelayW = getDBDataByFlag(inter);
            //DataTable dtDown = getDBDataByFlag(inter);
            this.chart1.Series[0].Points.Clear();
            //this.chart1.Series[1].Points.Clear();

            chart1.Series[0].Points.DataBind(tsnDelayW.AsEnumerable(), "time", "delay", "");
        ...
        private void wireless_Load(object sender, EventArgs e)
        {
            ...
            */
            ...
            //重新获取下行时延数据，绑定数据
            DataTable tsnDelayW = getDBDataByFlag(inter);
            //DataTable dtUp = getDBDataByFlag(inter);
